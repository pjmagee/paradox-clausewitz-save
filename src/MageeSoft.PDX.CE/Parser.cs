...
public class Parser
{
    ...
    public static SaveObject Parse(string input) => new Parser(input).Parse();

    private Parser(string input)
    {
        // ArgumentException.ThrowIfNullOrWhiteSpace(input, nameof(input)); // netstandard2.0 doesn't have this
    ...
    private SaveElement ParseValue()
    {
        ...
        if (_currentToken.Type == TokenType.StringLiteral)
        {
            ...
            if (DateTime.TryParseExact(text, "yyyy.M.d", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime yyyymd))
                return new Scalar<DateTime>(text, yyyymd);
            
            if (DateTime.TryParseExact(text, "yyyy.MM.d", CultureInfo.InvariantCulture, DateTimeStyles.None, out DateTime yyyymmd))
                return new Scalar<DateTime>(text, yyyymmd);
